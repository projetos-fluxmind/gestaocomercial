  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const [meRes2, defRes]: any[] = await Promise.all([
          apiFetch(`/api/companies/${meRes.company_id}/achievements/me`, { token: s.access_token }),
          apiFetch(`/api/companies/${meRes.company_id}/achievements`, { token: s.access_token }),
        ]);
      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
              Badges desbloqueados e disponíveis.
            </p>
          </div>
          <button
        ...
        <div className="mt-6 rounded-2xl border bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
            {unlocked.map((u) => (
              <div key={u.id} className="rounded-xl border border-green-200 bg-green-50 p-4 dark:border-green-900/50 dark:bg-green-950/30">
