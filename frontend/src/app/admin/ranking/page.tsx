...
export default function AdminRankingPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await apiFetch<ApiListResponse<LeaderboardEntry>>(`/api/companies/${meRes.company_id}/leaderboard`, {
  ...
  async function compute() {
    ...
    try {
      const meRes = await apiFetch<UserMe>('/api/users/me', { token: s.access_token });
      const res = await apiFetch<ApiListResponse<LeaderboardEntry>>(`/api/companies/${meRes.company_id}/leaderboard/compute`, {
        method: 'POST',
        token: s.access_token,
      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Leaderboard mensal (compute manual no MVP).</p>
          </div>
          <button
        ...
        <div className="mt-6 rounded-2xl border bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
          <h2 className="text-sm font-medium text-zinc-600 dark:text-zinc-400">Entradas ({rows.length})</h2>
            ...
            {rows.map((r) => (
              <div key={r.user_id} className="rounded-xl border p-4 dark:border-zinc-800">
                <div className="flex justify-between text-sm">
                  ...
                  <div className="text-zinc-600 dark:text-zinc-400">score {Number(r.performance_score).toFixed(4)}</div>
                </div>
                <div className="mt-2 text-xs text-zinc-600 dark:text-zinc-400">user_id: {r.user_id}</div>
              </div>
            ))}
          </div>
        </div>
